/* This library fuses position of a target from a 3D RGBD camera with acceleration from the IMU
 ...
 *  the state transition. The RGBD camera provides position information.
 *  
 *  We use 16 state variables which are the following:
 *  - 3 position of the target in the fixed frame    (X, Y, Z)
 *  - 3 velocity of the target w.r.t the fixed frame (X, Y, Z)
 *  - 4 orientation of the target in the fixed frame (quaternion W, X, Y, Z)
 *  - 3 accel bias variables (body frame)
 *  - 3 gyro bias variables  (body frame)
 *
 ...
 *  - 3 position of the target in the fixed frame
 *  - 4 orientation of the target in the fixed frame (quaternion W, X, Y, Z)
 *
 *  There are 7 measurement noise variables which are the following:
 ...
 *
 *  z = [p + n_p, qw + n_wq, qx + n_qx, qy + n_qy, qz + n_qz]
 *  where
 *  - p is the position in the world frame
 *  - n_p is the position noise
 *  - qw is the quaternion's w component
 *  - qx is the quaternion's x component
 *  - qy is the quaternion's y component
 *  - qz is the quaternion's z component
 *  - n_qw is the quaternion's w noise component
 *  - n_qx is the quaternion's x noise component
 *  - n_qy is the quaternion's y noise component
 *  - n_qz is the quaternion's z noise component
 *
 */

class TargetPositionIMU
{
